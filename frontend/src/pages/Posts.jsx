...
import { FileText, Plus, Filter, RefreshCw, Calendar, Share2, ExternalLink } from 'lucide-react';
...
export default function Posts() {
  ...
  // ════════════════════════════════════════════════════════════
  // Delete post — UPDATED FOR YOUTUBE NATIVE API REMOVAL
  // ════════════════════════════════════════════════════════════
  const handleDelete = async (postId, deleteFromPlatform = true) => {
    try {
      ...
      const isYoutubePost = targetPost?.platforms?.includes('youtube');
      ...
      if (isInstagramPost) {
        ...
        toast.success(
          deleteFromPlatform
            ? `Post deleted from app and platforms ${isYoutubePost ? '(including YouTube)' : ''}`
            ...
            {filteredPosts.map((post, index) => (
              ...
              >
                <div className="flex gap-6 flex-col md:flex-row">
                  ...
                  {/* Post Content & Live Platform Redirect Row */}
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    ...
                    {/* 🆕 NEW: Live Platform Action URLs Row */}
                    {post.platformResults && post.platformResults.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-slate-100">
                        {post.platformResults.map((result, rIdx) => {
                          // Only render button if a direct platform permalink is available
                          if (!result.permalinkUrl) return null;
